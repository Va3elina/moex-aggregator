...
ЦБ РЕВИЗИРУЕТ уже опубликованные месяцы между выпусками. Backfill хранит
САМУЮ СВЕЖУЮ ревизию: выпуски обрабатываются старые→новые, и через
ON CONFLICT DO UPDATE поздний выпуск перекрывает ранний. Детали — в
find_all_xlsx_urls().
...
def find_all_xlsx_urls() -> list[dict]:
    ...
    # участников между «НФО» и «Нефинансовыми организациями»).
    #
    # Политика: в БД лежит САМАЯ СВЕЖАЯ ревизия ЦБ. Выпуски обрабатываются в
    # прямом хронологическом порядке (старые → новые) — самый поздний выпуск,
    # содержащий месяц, пишется последним и побеждает. Годовой сводный выпуск
    # (псевдо-месяц 25) встаёт между декабрём и февралём следующего года и
    # перекрывается более поздними обычными выпусками.
