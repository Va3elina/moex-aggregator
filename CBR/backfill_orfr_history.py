...
Скачивает все XLSX с landing page (charts_ORFR_2023-04 … ORFR_2026-N) и парсит.
...
Backfill detect format по header и parse соответственно.

ЦБ РЕВИЗИРУЕТ уже опубликованные месяцы между выпусками. Чтобы в БД лежала
одна согласованная ревизия на год, источником истины берётся годовой сводный
выпуск (ORFR_YYYY-25-N): он обрабатывается ПОСЛЕДНИМ и через ON CONFLICT
DO UPDATE перекрывает первичные публикации. Детали — в find_all_xlsx_urls().
...
def find_all_xlsx_urls() -> list[dict]:
    ...
    # ── Порядок обработки = кто «победит» через ON CONFLICT DO UPDATE ──────
    # Последний записанный выпуск перезаписывает предыдущие. ЦБ публикует один
    # и тот же месяц по нескольку раз и РЕВИЗИРУЕТ его между выпусками — напр.
    # НФО за март-2024 выходил трижды: 18.6 → 14.4 → 17.1 (ревизия переносит
    # участников между «НФО» и «Нефинансовыми организациями», сумма по строке
    # сохраняется). Смесь первых публикаций и поздних правок в БД недопустима.
    #
    # Источник истины — годовой сводный выпуск ЦБ (ORFR_YYYY-25-N, псевдо-месяц
    # 25): он консолидирует все 12 месяцев года в единой ревизии. Поэтому
    # годовые выпуски обрабатываются ПОСЛЕДНИМИ и побеждают всё:
    #   • сначала обычные выпуски, новые→старые — для месяцев вне годовых
    #     выпусков (текущий неполный год) остаётся первая публикация;
    #   • затем годовые выпуски, старые→новые — свежий годовой перекрывает.
    def _processing_order(it: dict) -> tuple:
        is_annual = it["month"] > 12  # псевдо-месяц 25 = годовой сводный выпуск
        if is_annual:
            return (1, it["year"], it["month"])
        return (0, -it["year"], -it["month"])

    items.sort(key=_processing_order)
