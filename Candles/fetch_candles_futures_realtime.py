...
class CandlesUpdater:
    ...
    def _sync_instrument_prefixes(self) -> int:
        ...
        ⚠️ Раз в 15 минут МЕЖДУ ПРОЦЕССАМИ, через файл-отметку (запрос — 3,7 с;
        чаще не нужно, реже — новый контракт в день перехода дольше не виден на
        графиках: дневная свеча у него появляется с первого же прохода, так что
        задержка = этот интервал). Оркестратор запускает
        ...
        try:
            if отметка.exists() and datetime.now().timestamp() - отметка.stat().st_mtime < 900:
