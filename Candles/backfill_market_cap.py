...
cap = close(конец месяца) × ISSUESIZE (акций в обращении из ISS), В МЛРД РУБ.
ВАЖНО: stock_market_cap хранит капитализацию в МИЛЛИАРДАХ рублей (так пишет
дневной парсер Macro/fetch_market_cap.py из SmartLab). close×issuesize даёт
СЫРЫЕ рубли — поэтому делим на 1e9. Без деления возникал рассинхрон единиц:
бэкфилл-строки (рубли) перекрывали SmartLab-строки (млрд) у бумаг, которых
не было в свежем SmartLab-срезе, и heatmap раздувал их в гигантские плитки
(GAZP/NMTP/RASP/MRKP, инцидент 2026-06-12).

У нас в БД cap был только за 3 мес — этот скрипт достраивает историю для разрезов
стратегий по размеру компании. ISSUESIZE берётся ТЕКУЩИЙ (ISS не отдаёт историю
share count); для классификации крупная/средняя/малая этого достаточно (ранг
стабилен), но для бумаг с эмиссиями/байбэками абсолютная капа прошлого приблизительна.
...
def main():
    ...
    with eng.begin() as conn:
        for _, r in mon.iterrows():
            ...
            if not sz:
                continue
            cap = float(r["close"]) * sz / 1e9  # сырые рубли → млрд (единица таблицы)
            conn.execute(text("""
