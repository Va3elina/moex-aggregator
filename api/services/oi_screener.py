...
def _record_windows() -> Dict[str, str]:
    """Окна рекордов → ISO-дата начала окна. Горизонты: полгода + 1..5 лет +
    всё время. Месяц и 3 месяца убраны (решение Вадима): рекорд за такой срок
    не редкость, метка на нём обесценивает саму идею «пробили потолок».
...
_WINDOW_PERIODS = ("6m", "1y", "2y", "3y", "4y", "5y")
...
_RECORD_PERIODS = ("all", "5y", "4y", "3y", "2y", "1y", "6m")
...
def _record_for(net_pct, ex: Dict[str, Any] | None) -> Dict[str, str] | None:
    ...
    полгода), строго больше/меньше предыдущего экстремума. Короче полугода не
    считаем — такие «рекорды» слишком часты, чтобы быть сигналом.
