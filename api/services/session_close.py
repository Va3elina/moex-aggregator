...
def secid_closes(conn, secid: str, inst_type: str, date_from: date,
    ...
    ИНДЕКС. Фильтр по sec_id (плюс secid для точности) и без type — ключ
    покрывающего idx_candles_intraday_cover (sec_id, interval, begin_time)
    INCLUDE (…close, volume, secid), того же, по которому ходят графики
    (session_bars). type в INCLUDE не входит и вернул бы поход в таблицу; он и
    не нужен — у акций, индексов, валют и вечных фьючерсов secid == sec_id, а
    у датированных контрактов secid ('SRU6') ≠ sec_id ('SR'), так что пара
    однозначно задаёт бумагу (проверено по всей истории 5-минуток 2026-09-15).

    ⚠️ Индекс сам по себе не спасает. Замер 2026-09-15 на проде: оба варианта
    (старый по secid+type через idx_candles_stock_daily_secid_time и этот) шли
    Index Only Scan, но с Heap Fetches 10-19 тыс. и 3.5 с ввода-вывода: у
    свежих страниц candles не выставлен бит «все строки видимы», и за каждой
    пятиминуткой последних 60 дней Postgres всё равно шёл в таблицу. Таблица
    insert-only, автовакуум по умолчанию до неё почти не доходит. Лечится
    регулярным VACUUM (db/migrations/097_candles_oi_autovacuum_insert.sql);
    без него любой запрос по свежему интрадею, включая графики ОИ, платит
    диском на холодном кэше. Именно это тормозило линию цены в «Сделках фондов»
    у пользователя (админ живой версии этот запрос не делает).
    """
    start = datetime.combine(date_from, time.min)
