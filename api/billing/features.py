...
#   С 2026-09-11 цена наружу — только на закрытие 19:00 (services/session_close):
#   снапшот акций в strength.csv берёт цены карты рынка в версии зрителя,
#   seasonality.csv режет неопубликованный день и подменяет OHLC последних
#   недель сессией до 19:00, oi.csv отдаёт только позиции без цены (ОИ под
#   правило не подпадает).
#   ⚠️ Любая НОВАЯ выгрузка с ценой обязана идти через session_close
#   (is_live_viewer + publish_*), иначе наружу уйдёт живая цена.
