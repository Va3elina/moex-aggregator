...
from api.routers import agent_trace  # ← /api/admin/agent-trace/* (след агента для дашборда)
from api.routers import dashboard  # ← /api/admin/dashboard/* (карта проекта, снимок в Redis)
from api.routers import dashboard_posts  # ← /api/admin/dashboard/posts/* (разбор кандидата)
from api.routers import dashboard_graph  # ← /api/admin/dashboard/graph (граф владения)
...
# (interval, period) — дефолты ВСЕХ трёх поверхностей. Проверено замером:
# непокрытая комбинация стоит пользователю холодного счёта (SR 1день/вся —
# 3.9 с, N2 5м/6мес — 3.2 с), поэтому список идёт от того, что фронт реально
# просит на первом же экране:
#   сайт /oi   — 5м→6m, 1ч→1y, 1д→сохранённый период (обычно 1y, бывает all);
#   embed      — 1д→bestDailyPeriod, у Pro и admin это "all";
#   мобильная  — MAX_PERIODS_BY_INTERVAL режет интрадей месяцем: 5м→1m, 1ч→1m.
# Месячные и дневные срезы лёгкие (30-150 КБ против 900 КБ у 5м/6м), так что
# покрытие обходится примерно в 40 МБ сверху.
_WARM_CHART_COMBOS = (
    (5, "6m"), (5, "1m"),
    (60, "1y"), (60, "1m"),
    (24, "1y"), (24, "all"),
...
# Сколько САМЫХ ВОСТРЕБОВАННЫХ активов держим тёплыми.
#
# ⚠️ ПО УМОЛЧАНИЮ ПРОГРЕВ ВЫКЛЮЧЕН (0). Включается переменной окружения
# CHART_WARM_TOP_N без правки кода — чтобы можно было честно сравнить режимы
# на живом проде: сколько стоит прогрев и что без него теряет пользователь.
#
# Что известно на 2026-08-10:
#   • спрос резко неравномерный — SR 79 обращений за день, MX 31, дальше по
#     2-9; всего 19 активов из 91 доступных;
#   • прогрев ВСЕХ держал ~1100 ключей: cache_updater обходил их по 29 с
#     каждые 5 минут, load average уходил за 40;
#   • без прогрева первый зритель актива платит холодным счётом (1-6 с,
#     у SR до 23 с), но дальше ключ живёт сам — cache_updater продлевает TTL
#     всем существующим записям, и второй зритель получает его тёплым.
# Разумный компромисс, если включать: 12 активов ≈ 145 ключей.
_WARM_CHART_TOP_N = int(os.getenv("CHART_WARM_TOP_N", "0") or 0)

# Якорь на случай пустого счётчика спроса (перезапуск Redis): самые ликвидные
# фьючерсы MOEX. Как только накопится реальная статистика, она вытеснит этот
# список — он нужен лишь чтобы прогрев не простаивал на холодном старте.
_WARM_CHART_SEED = ("SR", "MX", "GZ", "Si", "BR", "USDRUBF")
...
def _warm_chart_cache():
    ...
    if _WARM_CHART_TOP_N <= 0:
        return          # прогрев выключен (см. CHART_WARM_TOP_N)
    ...
    try:
        ...
        if not assets:
            return
        # Греем ТОЛЬКО востребованное, а не все 91 видимых актива.
        #
        # Замер на проде 2026-08-10 (счётчик chartmeta:demand за день):
        #   SR 79 · MX 31 · GZ 9 · SV 8 · SS 7 · USDRUBF 4 · дальше по 2-3,
        #   всего 19 активов со спросом из 91 доступных.
        # Прогрев всех держал в кеше ~1100 ключей, из-за чего cache_updater
        # обходил их по 29 с каждые пять минут, а load average уходил за 40.
        # Платили ресурсами за активы, которые никто не открывал.
        #
        # Редкий актив теперь считается при первом обращении (1-6 с, у SR до 23),
        # а дальше живёт сам: cache_updater продлевает TTL всем существующим
        # ключам, так что второй и последующие зрители получают его тёплым.
        # Прогрев нужен ровно для того, чтобы ПЕРВЫЙ зритель популярного актива
        # не ждал — а популярных мало.
        seen = set(assets)
        hot = [a for a in top_demand() if a in seen][:_WARM_CHART_TOP_N]
        # Пустой счётчик (Redis перезапускали) → минимальный якорь по ликвидности,
        # чтобы прогрев не простаивал до первых обращений.
        order = hot or [a for a in _WARM_CHART_SEED if a in seen]
        if not order:
            return
...
app.include_router(dashboard.router)  # ← /api/admin/dashboard/* (карта проекта, снимок в Redis)
app.include_router(dashboard_posts.router)  # ← /api/admin/dashboard/posts/* (разбор кандидата)
app.include_router(dashboard_graph.router)  # ← /api/admin/dashboard/graph (граф владения)
