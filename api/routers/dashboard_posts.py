...
def карточка(
    ...
    return {
        ...
        "шаг_в_черновик": {
            ...
            # Строка FinanceMarker под постом с 10.09.2026 не публикуется, поэтому и в
            # превью её нет: превью обязано совпадать с тем, что уйдёт в канал.
            "аннотация": None,
...
def _вопросы_писателя(след) -> list[str]:
    """Вопросы писателя мозгу словами: «кто связан с X через владеет за 90 дней — 4 ответа»."""
    out = []
    # ⚠️ Автоматика тоже ходит в мозг: подсказка Шагу А (search по смыслу) и сборка
    # брифа (context по тикерам). Писателю принадлежит только то, что спрошено ПОСЛЕ
    # последней строки сборки брифа — иначе в «вопросы писателя» попадёт и код.
    # Строки шага «бриф» пишутся в момент запуска писателя; всё, что мозг ответил
    # позже, спрашивал уже писатель (или судья — у него тот же бриф, но он не спрашивает).
    граница = max([t["created_at"] for t in след if t.get("step") == "бриф" and t.get("created_at")],
                  default=None)
    for t in след:
        q = (t.get("question") or "")
        if t.get("step") != "мозг" or not q:
            continue
        if граница and t.get("created_at") and t["created_at"] <= граница and q.split(" ")[0] in ("search", "context"):
            continue
        слово = q.split(" ")[0]
        n = t.get("result_count") or 0
        итог = f"{n} {'ответ' if n == 1 else 'ответа' if 1 < n < 5 else 'ответов'}"
        if слово == "neighbors":
            m = re.search(r"neighbors (\S+) kind=(\S+) since=(\S+)", q)
            if m:
                out.append(f"Кто связан с {m.group(1)} через «{m.group(2)}» за {m.group(3)} дн. — {итог}.")
        elif слово == "path":
            m = re.search(r"path (\S+) → (\S+)", q)
            if m:
                out.append(f"Есть ли связь между {m.group(1)} и {m.group(2)} — " + (f"{итог}: {t.get('result_note')}" if t.get("result_note") else итог) + ".")
        elif слово == "search":
            out.append(f"Поиск по новостям базы: {q[len('search '):]} — {итог}.")
        elif слово == "similar":
            out.append(f"Похожие на {q.split(' ')[1]} — {итог}.")
        elif слово == "node":
            out.append(f"Карточка {q.split(' ')[1]} — {итог}.")
        elif слово == "context" and граница and t.get("created_at") and t["created_at"] > граница:
            m = re.search(r"context (\S+) days=(\S+)", q)
            if m:
                out.append(f"Срез по {m.group(1)} за {m.group(2)} дн. — {итог}.")
    return out
...
def _рассказ(c, новость, аномалия, след, журнал) -> list[dict]:
    ...
    if c["draft_text"] or c["synth_declined_reason"] or бриф:
        ...
        # Уточняющие вопросы писателя мозгу (с 06.09 промпт разрешает до трёх). Их
        # видно только по следу: строки шага «мозг» с вопросами neighbors/path/search/
        # similar/node, которые не относятся к сборке брифа. Показываем словами.
        вопросы = _вопросы_писателя(след)
        if вопросы:
            шаги.append({"код": "шаг_в_мозг", "заголовок": "Писатель спросил у мозга",
                         "строки": вопросы, "тон": "neutral"})
    # 7. судья черновика
    if c["judge_verdict"]:
