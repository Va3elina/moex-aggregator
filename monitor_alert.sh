...
# pipeline_heartbeat). «Проблемный» = last_status<>'ok' ИЛИ не запускался >26ч
# (ловит и упавший скрипт, и полностью вставший оркестратор/SPOF). Все текущие
# heartbeat-пайплайны — daily/5-мин, недельных нет → порог 26ч без ложных.
# ─────────────────────────────────────────────────────────────
...
ROWS=$(docker exec "$CONTAINER" psql -U postgres -d moex_db -tA -F'|' -c \
  "SELECT pipeline, last_status, round(EXTRACT(EPOCH FROM now()-last_run_at)/3600, 1)
   FROM pipeline_runs
   WHERE last_status <> 'ok' OR last_run_at < now() - interval '26 hours'
