...
# Учёт вступлений, снимок подписчиков и алерты живут в signals/tg_joins.py и
# крутятся в signals/content_review_bot.py: с прода контейнер до
# api.telegram.org не достаёт (IPv6-only адрес без маршрута из docker-сети),
# поэтому этот бот на проде апдейтов не получает.
