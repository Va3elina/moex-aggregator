...
## 6. ⚠️ CSP в rules.json — снимок политики T-Bank (требует пересинхрона)
`rules.json` не удаляет CSP терминала, а ЗАМЕНЯЕТ его (operation: `set`) на
**точную копию** политики T-Bank с добавленным `таймфрейм.рф` в `frame-src`
(подход A, чтобы сохранить script-src/connect-src и пройти CWS-ревью). Копия
снята 2026-06-12 с `https://www.tbank.ru/invest/web-terminal/`.

**Если терминал начал глючить ИЛИ панели Фрейма перестали грузиться** (T-Bank
поменял свой CSP, наша копия устарела и перекрывает его) — пересними:
```bash
curl -s -A "Mozilla/5.0 Chrome/120" -D - -o /dev/null \
  https://www.tbank.ru/invest/web-terminal/ | grep -i '^content-security-policy:'
```
Вставь актуальное значение в `value` всех правил rules.json, добавив
`https://xn--80aklbnczmv.xn--p1ai ` сразу после `frame-src `. Подними version,
republish. (Скрипт-генератор был в истории сессии 2026-06-12.)
