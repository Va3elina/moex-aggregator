...
/** Палитра поповера цвета: верхний ряд — тона тёмной темы, нижний — светлой.
 *  Цветная часть рядов ВЗЯТА С ЛИНИЙ ГРАФИКА (открытые позиции), чтобы разметка
 *  попадала в цвет серии: фиолет = «Продажи» (--oi-red), зелёный = «Покупки»
 *  (--oi-green), синий = цена (--chart-line-1), оранжевый = ОИ (--chart-line-2),
 *  pumpkin = «Чистая позиция» (--accent / --oi-cyan).
 *  Держим ЛИТЕРАЛАМИ, а не токенами темы: цвет фигуры сохраняется в персист и
 *  должен остаться тем же при смене темы/экспорте (токен бы «поплыл»). */
const PALETTE_ROWS: string[][] = [
  ['#F5F1E8', '#C9C4B8', '#9A958C', '#6B6760', '#0A0A0A', '#EF6F6F', '#FF5C2B', '#F2A24A', '#5BD49C', '#57C7C7', '#5DA3E9', '#B91C5C'],
  ['#FFFFFF', '#E3DED2', '#807B72', '#3A3833', '#1C1B20', '#B14A4A', '#C2451F', '#E07A1F', '#0A6B3B', '#3A8C8C', '#2B6CB0', '#8E1746'],
];
