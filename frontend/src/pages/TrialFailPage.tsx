...
export default function TrialFailPage() {
  ...
  return (
    <div style={{
      ...
      <h1 style={{ fontSize: 'var(--fs-2xl)', marginBottom: 12 }}>{t('Не удалось активировать пробный период')}</h1>
      <p style={{ color: 'var(--text-secondary, #666)', marginBottom: 8, lineHeight: 1.6 }}>
        {t('Карту не удалось привязать — банк отклонил операцию или вы отменили её.')}
        <b> {t('Деньги не списаны.')}</b> {t('Пробный период не активирован.')}
      </p>
      <p style={{ color: 'var(--text-secondary, #666)', marginBottom: 20, lineHeight: 1.6 }}>
        {t('Можно попробовать ещё раз — при привязке карта проверяется на 0 ₽, списание происходит только по окончании пробного периода.')}
      </p>
      ...
      <div style={{ marginTop: 20, fontSize: 'var(--fs-sm)', color: 'var(--text-muted, #999)' }}>
        {t('Вопросы?')} <a href="mailto:frameinfo@mail.ru" style={{ color: 'var(--accent, #FF5C2B)' }}>frameinfo@mail.ru</a>
      </div>
    </div>
  );
}
