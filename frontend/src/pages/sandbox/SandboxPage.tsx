...
const SIZES: Partial<Record<IndKind, { w: number; h: number }>> = {
  ...
  // Замер вживую: вкладка + бумага + фонды + 3 режима + 3 периода + хром окна
  // просят ~760px; ниже полоса контролов уходит в скролл (не ломается, но
  // периоды прячутся). Высота 520 — чтобы влезли оба графика с осью и навигатором.
  'fund-trades': { w: 760, h: 520 },
  // Потоки фондов теперь двухпанельные (бенчмарк сверху, гистограмма снизу) —
  // на дефолтных 560×400 обе секции сплющивались, а даты оси слипались.
  // 640×520 — тот же бюджет высоты, что у «Сделок фондов» с их двумя секциями.
  'funds-money': { w: 640, h: 520 },
};
...
const MINW_BY_TYPE: Partial<Record<IndKind, number>> = {
  ...
  // Вкладка «Потоки» кладёт свои контролы (бумага/режим/период/фонды) в ту же
  // строку тулбара, что и таб-бар — 490 стало мало.
  'fund-trades': 620,
