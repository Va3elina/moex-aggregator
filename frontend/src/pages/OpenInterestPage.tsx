...
const OI_ALERT_METRICS: AlertMetricOption[] = [
  ...
  },
  {
    ...
    // который «резкое движение» (ATR) пропускает. op = направление (макс/мин),
    // period = окно (уходит в threshold как дни: 30/90/0=всё время).
    key: 'extreme_fiz', label: 'Новый максимум/минимум позиции — физлица',
    indicator: 'oi_extreme', metric: 'net', clgroup: 'FIZ',
    ops: [{ value: 'new_high', label: 'новый максимум (лонг-рекорд)' },
          { value: 'new_low', label: 'новый минимум (шорт-рекорд)' }],
    periodControl: [{ value: '180', label: 'за 6 месяцев' },
                    { value: '365', label: 'за год' },
                    { value: '0', label: 'за всё время' }],
    hint: 'Сработает, когда чистая позиция физлиц достигнет нового перекоса-рекорда (максимума лонга или минимума — рекордного шорта) за выбранный период. В отличие от «резкого движения», ловит МЕДЛЕННЫЙ дрейф: позиция может ползти неделями и поставить рекорд без единого резкого дня.',
  },
  {
