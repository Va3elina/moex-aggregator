...
export interface Candle {
  ...
  live?: boolean;
  /** Растянутая цена: свеча раздаётся не сразу (публично — только закрытие
   *  19:00, у админа — с задержкой 15 минут), а ОИ актуален — на этом отрезке
   *  продлено последнее известное значение
   *  (chart_live.append_stretched_price). Заменяется фактической свечой, когда
   *  та доедет. */
  stretched?: boolean;
}
...
export interface ChartResponse {
  ...
  /** Какую цену несёт ответ (бэкенд services/session_close): 'live' —
   *  незамедленная (админ), 'pub' — только закрытие 19:00; на интрадей это
   *  ступенька open = high = low = close. */
  price_view?: 'live' | 'pub';
}
