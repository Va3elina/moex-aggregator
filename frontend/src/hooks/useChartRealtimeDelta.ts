    ...
    useRealtimeData(channels, () => {
        ...
        fetchAndMergeChartDelta(cur)
            .then((next) => {
                ...
                //
                // ⚠️ Сверяем именно NEXT, а не ref.current.data. Прежняя проверка
                // спрашивала «совпадает ли ТЕКУЩИЙ срез с текущими контролами» —
                // а он совпадал всегда, если полный рефетч нового ТФ успел
                // завершиться. Устаревшая дельта старого ТФ проходила гард и
                // перетирала свежие данные: график 1ч показывал 5-минутный ряд
                // до следующего рефетча.
                if (next && matchesControls(next, ref.current.controls)) {
