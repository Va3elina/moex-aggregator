...
export const ANIMATION = {
   ...
   *  ещё анимируется. Выше — пути ставятся мгновенно.
   *
   *  Замер в браузере 2026-08-10 (кадр = интерполяция всех точек + сборка
   *  d-строки, три серии): 2 тыс. точек — 1.0 мс, 4.8 тыс. — 1.8 мс,
   *  26.7 тыс. — 19.1 мс при бюджете кадра 16.7 мс. То есть на 5-минутках
   *  морфинг рвётся даже на быстрой машине (отсюда исходная жалоба на
   *  «дёрганье»), а на часовом и дневном стоит копейки — гасить его там
   *  значило бы менять плавный ход линии на рывок без всякой выгоды.
   *  Запас ×3 на слабое железо: 8 тыс. точек ≈ 3 мс/кадр, на медленном
   *  ноутбуке ~9 мс — всё ещё внутри бюджета. */
  appendMorphMaxPoints: 8000,
