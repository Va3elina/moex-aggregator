...
export const ISIN_TO_TICKER: Record<string, string> = {
    ...
    'RU000A10CTQ0': 'BAZA',
    'RU000A10CW95': 'OZON',
    'US33835G2057': 'FIXR',   // ГДР Fix Price, делистинг 2025 → акция ФиксПрайс
    'US37949E2046': 'GLTR',
    'US42207L1061': 'HEAD',
    'US5603172082': 'VKCO',
    'US69269L1044': 'OZON',
...
// которые есть в портфелях фондов. Без записи здесь показывается сырое имя из
// отчёта фонда (у части бумаг это «ПАО …»-полотно). Курируется ВРУЧНУЮ — не из БД.
 ...
 * (OFZ_GERB, тот же что у индекса RGBI в InstrumentIcon) вместо безликой точки.
 ...
 *
 * ВАЖНО: не использовать \b после «ОФЗ» — в JS \b работает по ASCII-классу \w,
 * а после кириллической «З» границы слова нет, поэтому /ОФЗ\b/ НИКОГДА не
 * матчит «ОФЗ 26238» (был баг — герб не появлялся). Матчим «ОФЗ» + разделитель.
 */
export function isOfzBond(name?: string | null): boolean {
    return /^\s*ОФЗ(?:[\s\d-]|$)/i.test(name || '');
}
