...
export interface AdminPerson {
  ...
  email: string | null;
  role: string | null;
  /** guest — без аккаунта; reg — с аккаунтом; invite — подарочная подписка; paid — платит. */
  kind: 'guest' | 'reg' | 'invite' | 'paid';
  plan: string | null;
  plan_period: string | null;
  plan_expires_at: string | null;
  days: number;
...
export interface PeopleReport {
  ...
  prev_people: number;
  /** С какой страницы человек начал: для прямых заходов это единственный
   *  способ понять, что именно им было нужно. */
  entry_pages: { path: string; name: string; people: number }[];
  sources: { source: string; people: number }[];
  limit: number;
  /** Сколько людей под текущим отбором (до потолка выдачи). */
  matched: number;
  /** Счётчики по периоду без kind и без отбора по индикаторам — подписи кнопок. */
  counts: {
    people: number; guests: number; registered: number; loyal: number;
    loyal_guests: number; paid: number; alerts: number; tg: number;
    visits: number; views: number;
  };
  indicators: { path: string; name: string; people: number; registered: number }[];
  all_indicators: { path: string; name: string }[];
  people: AdminPerson[];
...
export async function listPeople(opts: AdminRange & SegmentParams & {
  segment?: string;
  device?: string;
  /** all / guest / reg / loyal / paid / alerts */
  kind?: string;
  /** days / visits / views / last */
  sort?: string;
  ...
  if (opts.sort) params.set('sort', opts.sort);
  const response = await apiFetch(`${API_BASE}/api/analytics/people?${params}`);
  if (!response.ok) {
    if (response.status === 403) throw new Error(t('Доступ только для администратора'));
...
export interface RevenueReport {
  payers: number; payments: number; rub: number;
  payers_30: number; rub_30: number;
  active_paid: number; registered: number;
  by_month: { month: string; payers: number; rub: number }[];
  mix: { tier: string; period: string; n: number; rub: number }[];
  repeat: { payments: number; users: number }[];
}

export async function getRevenue(): Promise<RevenueReport> {
  const response = await apiFetch(`${API_BASE}/api/analytics/revenue`);
  if (!response.ok) {
    if (response.status === 403) throw new Error(t('Доступ только для администратора'));
