...
// ════════════════════════════════════════════════════════════════════════════
// Watchlist — личный список избранных тикеров для overview
// ════════════════════════════════════════════════════════════════════════════
...
export async function getWatchlist(): Promise<WatchlistItem[]> {
  ...
  if (response.status === 401) return []; // Гость — пустой watchlist.
...
export async function addToWatchlist(ticker: string): Promise<WatchlistItem> {
  ...
  if (!response.ok) {
    if (response.status === 401) throw new Error('Войдите чтобы добавить в избранное');
    if (response.status === 400) {
      const data = await response.json().catch(() => ({}));
      throw new Error(data.detail || 'Не удалось добавить');
    }
    if (response.status === 404) throw new Error(`Инструмент ${ticker} не найден`);
...
export async function removeFromWatchlist(ticker: string): Promise<void> {
  const response = await apiFetch(`${API_BASE}/api/watchlist/${encodeURIComponent(ticker)}`, {
    method: 'DELETE',
  });
  if (!response.ok && response.status !== 204) {
...
export async function reorderWatchlist(tickers: string[]): Promise<void> {
  const response = await apiFetch(`${API_BASE}/api/watchlist/reorder`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ tickers }),
  });
  if (!response.ok) throw new Error('Не удалось переупорядочить');
}

// ════════════════════════════════════════════════════════════════════════════
// Screeners — preset-фильтры для overview
// ════════════════════════════════════════════════════════════════════════════
...
export async function getScreenerPresets(): Promise<ScreenerPreset[]> {
  const response = await apiFetch(`${API_BASE}/api/screeners`);
  if (!response.ok) throw new Error('Failed to fetch screener presets');
  return response.json();
}

export async function runScreener(presetId: string): Promise<ScreenerItem[]> {
  const response = await apiFetch(`${API_BASE}/api/screeners/${encodeURIComponent(presetId)}`);
  if (!response.ok) throw new Error(`Failed to run screener ${presetId}`);
  return response.json();
}
