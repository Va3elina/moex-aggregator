...
// ==================== ADMIN: МЕЖДУНАРОДНЫЙ OI (только admin) ====================

export interface OiIntlAsset {
  exchange: string;
  country: string;
  asset_code: string;
  asset_name: string;
...
export interface OiIntlHistoryPoint {
  date: string;
  oi_long: number | null;
  oi_short: number | null;
  oi_total: number;
  /** 'daily'|'weekly'|'monthly' — на датах, где источников несколько (напр. Eurex с 2022-08), backend отдаёт самую мелкую доступную по умолчанию. */
  granularity: string;
}

export interface OiIntlHistoryResponse {
  exchange: string;
  asset_code: string;
  category: string;
  data: OiIntlHistoryPoint[];
}

export interface OiIntlCandlePoint {
  date: string;
  open: number | null;
  high: number | null;
  low: number | null;
...
export interface OiIntlCandlesResponse {
  exchange: string;
  asset_code: string;
...
export async function getOiIntlCategories(exchange: string, assetCode: string): Promise<{ categories: string[] }> {
  const params = new URLSearchParams({ exchange, asset_code: assetCode });
  const response = await apiFetch(`${API_BASE}/api/admin/oi-intl/categories?${params}`);
  if (!response.ok) throw new Error('Failed to fetch international OI categories');
  return response.json();
}

export async function getOiIntlHistory(
  exchange: string,
  assetCode: string,
  category: string = 'TOTAL',
  days: number = 730,
): Promise<OiIntlHistoryResponse> {
  const params = new URLSearchParams({ exchange, asset_code: assetCode, category, days: days.toString() });
  const response = await apiFetch(`${API_BASE}/api/admin/oi-intl/history?${params}`);
  if (!response.ok) throw new Error('Failed to fetch international OI history');
  return response.json();
}

export async function getOiIntlCandles(
  exchange: string,
  assetCode: string,
  days: number = 730,
): Promise<OiIntlCandlesResponse> {
  const params = new URLSearchParams({ exchange, asset_code: assetCode, days: days.toString() });
  const response = await apiFetch(`${API_BASE}/api/admin/oi-intl/candles?${params}`);
  if (!response.ok) throw new Error('Failed to fetch international candles');
  return response.json();
}
...
export interface CompanyPriceWeeklyResponse {
    ...
    /** Медианный дневной оборот (₽) за скользящее окно «месяц + два
     *  предыдущих», ТОЛЬКО будние сессии, выровнено с turnover_months —
     *  знаменатель режима «Навес» (дней на выход из позиции). Окно
     *  трёхмесячное, потому что месячная медиана шумит на порядок сильнее
     *  числителя; выходные торги МосБиржи исключены, ПИФы в них не участвуют.
     *  См. price_weekly. */
    med_turnover?: number[];
}
