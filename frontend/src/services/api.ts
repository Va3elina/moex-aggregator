...
export interface CompanyPriceWeeklyResponse {
     ...
     *  предыдущих», выровнено с turnover_months — знаменатель режима «Навес»
     *  (дней на выход из позиции). Окно трёхмесячное, потому что месячная
     *  медиана шумит на порядок сильнее числителя, см. price_weekly. */
    med_turnover?: number[];
}
