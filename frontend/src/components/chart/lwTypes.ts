...
export interface LwSeries {
  ...
  zeroLine?: boolean;
  /** Показывать в строке легенды изменение к предыдущему бару (±Δ и ±%). Имеет
   *  смысл там, где ряд меряет УРОВЕНЬ — цена, открытый интерес. У осцилляторов
   *  оно шум: RSI даёт «+0» (это пункты индекса), объёмы — «+176%» между
   *  соседними барами. Флаг у ряда, а не «панель 0»: ОИ умеет уезжать вниз,
   *  и изменение обязано уехать вместе с ним. */
  legendChange?: boolean;
