...
export default function MobileChart({
  ...
  // Правая ось — ФИКСИРОВАННЫЙ жёлоб (решение владельца 2026-09-07, единый
  // стандарт всех мобильных графиков: «Сила рынка», «Поток капитала», «Деньги
  // в фондах», «Сделки фондов»): линия/бары рисуются только до W - жёлоб, а
  // подписи шкалы и pill стоят в чистой колонке справа и НЕ перекрываются
  // графиком. Ширина — под ~6 символов подписи («150.0K», «-50.0K») при
  // текущем размере шрифта оси + зазор. Оверлей поверх графика (#329) откачен.
  const PILL_GUTTER_R = hasRightAxis ? Math.ceil(6 * axisFs * 0.62) + 8 : PAD_X;
