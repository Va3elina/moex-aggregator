    ...
    const deskRow = (h: FundPortfolioHolding, idx: number, last: boolean, interactive: boolean) => {
        ...
        return (
            ...
            >
                ...
                <span style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)' }}>{w.toFixed(2)}%</span>
                <span style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)' }}>{fmtVolBln(h.value_rub)}</span>
                <span style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)' }}>{h.funds_holding}</span>
            </div>
        );
    };
