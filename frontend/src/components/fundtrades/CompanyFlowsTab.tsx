 ...
 *    оборот торгов за скользящие три месяца (med_turnover из /price-weekly;
 *    окно трёхмесячное, потому что месячная медиана шумит на порядок сильнее
 *    числителя — см. price_weekly). Пункт скрыт, если оборота нет (не акция /
 *    нет истории в candles).
...
const MODE_HELP: Record<ChartMode, string> = {
    ...
    overhang: 'Позиция выбранных фондов, делённая на медианный дневной оборот торгов за последние три месяца. Показывает, за сколько дней торгов фонды могли бы выйти из бумаги, забирая весь оборот: чем больше дней, тем труднее им двигаться без влияния на цену.',
};
