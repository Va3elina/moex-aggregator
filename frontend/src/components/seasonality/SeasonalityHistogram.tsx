...
export default function SeasonalityHistogram({
  ...
  return (
    <div className={`relative overflow-hidden pb-8 cursor-crosshair ${revealed ? 'chart-reveal' : ''}`} style={{ aspectRatio: '2.4', minHeight: 280, maxHeight: 550 }}
      onMouseMove={(e) => {
        const rect = e.currentTarget.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const PR_PX = 80;
        const barAreaWidth = rect.width - PR_PX;
        if (x < 0 || x > barAreaWidth) { setTooltip(null); return; }
        const slotW_px = barAreaWidth / bars.length;
        const idx = Math.floor(x / slotW_px);
        if (idx >= 0 && idx < bars.length) {
          const slotCenterPx = idx * slotW_px + slotW_px / 2;
      ...
      <div className="absolute inset-0" style={{ right: 80, top: 28 }}>
