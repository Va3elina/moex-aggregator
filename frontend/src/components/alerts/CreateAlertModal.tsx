...
const alertsWord = (n: number) => t(plural(n, 'уведомление', 'уведомления', 'уведомлений'));
const assetsWord = (n: number) => t(plural(n, 'актив', 'актива', 'активов'));
...
export default function CreateAlertModal({ indicator, asset, assetName, metrics, onClose, prefill }: Props) {
    ...
    // Вне песочницы контекстная тема и корневая совпадают — хук молчит, и на
    // сайте оверлей остаётся ровно таким, каким был.
    ...
    const handleCreate = async () => {
        if (!metric) return;
        if (channelsArr.length === 0) { setMsg({ type: 'err', text: t('Выберите хотя бы один канал доставки') }); return; }
        if (needsTelegramLink) { setMsg({ type: 'err', text: t('Подключите Telegram или уберите этот канал') }); return; }
        ...
        if (!isTierMetric) {
            ...
            if (Number.isNaN(th)) { setMsg({ type: 'err', text: t('Введите числовой порог') }); return; }
            setBusy(true); setMsg(null);
            try {
                const payload: AlertCreatePayload = {
                    indicator: metric.indicator, asset, asset_name: assetName,
        ...
        if (selectedList.length === 0) { setMsg({ type: 'err', text: t('Выберите хотя бы один актив') }); return; }
        ...
        try {
            ...
            if (res.created > 0) {
                ...
                setCreated(true);
                const notes: string[] = [];
                if (res.skipped > 0) notes.push(t('{{n}} уже были', { n: res.skipped }));
                if (res.errors.length) notes.push(t('не удалось {{n}}: {{err}}', { n: res.errors.length, err: res.errors[0] }));
                if (notes.length) setMsg({ type: 'err', text: notes.join('; ') });
            } else if (res.skipped > 0) {
                setMsg({ type: 'ok', text: t('Эти уведомления уже созданы') });
            } else {
                setMsg({ type: 'err', text: res.errors[0] || t('Не удалось создать уведомления') });
            }
        } catch (e) {
            setMsg({ type: 'err', text: (e as Error).message });
        } finally {
    ...
    return createPortal(
        <div {...portalTheme} style={{ ...overlay, ...portalTheme.style }} onClick={onClose}>
            <div style={card} onClick={(e) => e.stopPropagation()}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 'var(--fs-lg)' }}>
                        <AlarmClock size={20} style={{ color: 'var(--accent)' }} /> {t('Новое уведомление')}
                    </span>
                ...
                {created ? (
                    <div style={{ textAlign: 'center', padding: '12px 0' }}>
                        ...
                        <div style={{ fontWeight: 600, marginBottom: 4 }}>
                            {createdCount > 1
                                ? t('{{n}} {{word}} создано', { n: createdCount, word: alertsWord(createdCount) })
                                : t('Уведомление создано')}
                        </div>
                        <div style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-sm)', marginBottom: 16 }}>
                            {createdCount > 1 ? t('Уведомления придут') : t('Уведомление придёт')}{' '}
                            {[
                                channels.has('telegram') && t('в Telegram'),
                                channels.has('site') && t('на сайт'),
                                channels.has('email') && t('на e-mail'),
                            ].filter(Boolean).join(t(' и ')) || t('на сайт')}.
                        </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        ...
                        {isTierMetric ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{t('Активы')}</div>
                                ...
                                >
                                    <Search size={18} /> {t('Выбрать активы')}
                                </button>
                            ...
                            <div style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-sm)' }}>
                                {t('Актив:')} <b style={{ color: 'var(--text-primary)' }}>{assetName || asset}</b>
                                {/* «Сейчас»: из «+» приходит currentLabel (цена ₽ / ОИ контракты);
                                    иначе для price-метрики — из контекста свежей цены. */}
                                {prefill?.currentLabel ? (
                                    <>
                                        {' · '}{t('Сейчас:')} <b style={{ color: 'var(--text-primary)' }}>{prefill.currentLabel}</b>
                                    </>
                                ) : hasPrice && metric?.indicator === 'price' && (
                                    <>
                                        {' · '}{t('Сейчас:')} <b style={{ color: 'var(--text-primary)' }}>{fmtRub(price!.value!)} {t('руб')}</b>
                                    </>
                                )}
                            </div>
                        )}
                        ...
                        <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{t('Метрика')}
                        ...
                        {isTierMetric ? (
                            ...
                            <>
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{t('Условие')}
                                ...
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{t('Период')}
                            ...
                            <>
                                ...
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>
                                    {t('Порог')} {metric?.unit ? `(${metric.unit})` : ''}
                        ...
                        {isTierMetric && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{t('Когда проверять')}</div>
                                ...
                                <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                                    {t('5 мин и 1 час ловят дневной сдвиг раньше — доступны у ликвидных активов (с бейджем). У остальных всегда раз в день.')}
                                </div>
                            </div>
                        )}
                        ...
                        {/* ── КАНАЛЫ ДОСТАВКИ ─────────────────────────────────── */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{t('Куда присылать')}</div>
                            <div style={{ display: 'flex', gap: 6 }}>
                                {([
                                    ...
                                    return (
                                        ...
                                        >
                                            {t(c.label)}{c.soon && <span style={{ fontWeight: 600, opacity: 0.8 }}> · {t('скоро')}</span>}
                                        </button>
                                    );
                                })}
                            </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {linkUrl ? (
                                        <>
                                            ...
                                            <div style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-xs)', textAlign: 'center' }}>{t('Нажмите Start в боте — статус обновится сам')}</div>
                                        </>
                                    ) : (
                        ...
                        {isTierMetric && (
                            <div style={{
                                ...
                                <div style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>
                                    {t('Как считается резкость')}
                                </div>
                                {t('Берётся отслеживаемая величина и её изменение за день. Это изменение делится на средний дневной шаг за последние 14 торговых дней. Получается «во сколько раз сегодняшнее движение больше обычного»: 1 — обычный день, 3 — втрое сильнее обычного. Уровни откалиброваны по частоте на ликвидных бумагах: «Заметное» случается примерно раз в неделю, «Сильное» — раз в две-три недели, «Экстремальное» — раз в два месяца. Уведомление описывает само движение, а не прогноз цены.')}
                            </div>
                        )}
                        ...
                        {metric?.indicator === 'price' && context && price?.intraday === false && (
                            <div style={{
                                ...
                                <span>{t('У этого актива нет внутридневных данных — проверка раз в день после закрытия.')}</span>
                            </div>
                        )}
                        ...
                        {isTierMetric && (
                            <div style={{
                                ...
                                <Info size={16} style={{ flex: '0 0 auto', marginTop: 1, color: 'var(--accent)' }} />
                                <span>
                                    {t('Для ликвидных активов можно ловить дневной сдвиг раньше — по часовым или 5-минутным данным (таймфрейм выше). У остальных — раз в день. Это описание движения, а не прогноз цены.')}
                                </span>
                            </div>
                        )}
                        ...
                        {isTierMetric && (
                            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>
                                ...
                                {' × '}
                                <b style={{ color: 'var(--text-primary)' }}>
                                    {isCustomLevel ? `${customMult || '—'}×` : t(activeLevel?.label ?? '')}
                                </b>
                                {' = '}
                        ...
                        >
                            {busy
                                ? t('Создаём…')
                                : isTierMetric
                                    ? t('Создать {{n}} {{word}}', { n: selectedList.length, word: alertsWord(selectedList.length) })
