...
// Масштаб линеен по ширине графика в обе стороны: узкий снимок (суженное окно
// песочницы) без даунскейла оставлял header/footer фиксированных 28/15/14px,
// и подписи забирали всё внимание с самого графика. Снизу клампим MIN_LAYOUT_SCALE,
// чтобы на совсем узких панелях текст не вырождался в нечитаемый.
//
...
function computeLayoutScale(chartW: number, dpr: number): number {
    return Math.max(MIN_LAYOUT_SCALE, chartW / (REFERENCE_W * dpr));
}
