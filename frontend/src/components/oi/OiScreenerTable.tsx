...
// Подписи по горизонту сигнала («Краткосрочный» / «Среднесрочный»), конкретный
// срок вынесен первой строкой тултипа — чтобы за какой период посчитано движение
// было видно по наведению, а сами кнопки не привязывались к точным цифрам окна.
const HORIZON_OPTIONS = [
  { key: 'short' as const, label: 'Краткосрочный', title: 'Горизонт — один торговый день. Движение позиции за день против её обычного дневного размаха (ATR-14). Это те же сигналы, что уходят в Telegram-алерты.' },
  { key: 'medium' as const, label: 'Среднесрочный', title: 'Горизонт — 14 торговых дней (около двух недель). Сдвиг позиции за этот срок против того, сколько актив обычно проходит за такой же период. Ловит то, что копилось неделями и в дневной ленте не видно.' },
];
