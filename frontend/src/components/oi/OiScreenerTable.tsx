 ...
 * порога «резко»: ×1 контур цветом текста, ×1,5 контур accent, ×2 (или
 * сильнейшая строка выборки от ×1,5) заливка accent с кольцом. Относительный градиент «теплоты» пробовали и убрали: он
...
// Подписи по горизонту сигнала («Дни» / «Недели»), конкретный срок вынесен в
// тултип и в «?» рядом с тумблером — чтобы за какой период посчитано движение
// было видно по наведению, а сами кнопки не привязывались к точным цифрам окна.
...
const HORIZON_OPTIONS = [
  { key: 'short' as const, label: 'Дни', title: 'Горизонт — один торговый день. Движение позиции за день против её обычного дневного размаха (ATR-14). Это те же сигналы, что уходят в Telegram-алерты.' },
...
// Пояснение к тумблеру горизонта («?» внутри пилюли). Третьим блоком — про лаг
// данных: дневной срез позиций по большинству фьючерсов доступен только на
// следующий торговый день, и без этой оговорки «Дни» читаются как «сегодня».
const HORIZON_HELP = [
  { heading: 'Дни', body: 'Движение позиции за один торговый день против обычного дневного размаха актива (ATR-14). Ловит резкие однодневные сдвиги. Это те же сигналы, что уходят в Telegram-алерты.' },
  ...
  { heading: 'Свежесть данных', body: 'Считаем по дневным срезам позиций, а они по большинству фьючерсов публикуются на следующий торговый день. По активам с внутридневными данными последняя точка обновляется в течение дня. Дата расчёта — справа в тулбаре.' },
];
...
export default function OiScreenerTable({ onSelect, onRequestAlert }: Props) {
  ...
  // текста), x1,5 — сильная (контур accent), x2 — особо сильная (заливка
  // accent + кольцо). Градиент «теплоты» убран: он красил лидера тихого дня
  // так же ярко, как настоящий выброс, и одинаковое число в разные дни
  ...
  const ratioCell = (r: OiScreenerRow) => {
    ...
    // Заливка: либо абсолютный выброс (2 порога — 4× за день, 6× за 2 недели),
    // либо лидер выборки, если он хотя бы в 1,5 порога. Второе условие нужно,
    // потому что чисто абсолютный порог не брался целыми днями (типичный
    // максимум дня 3,5…4,5× при пороге «резко» 2×) — заливки просто не было
    // видно. Теперь сильнейшая строка дня залита почти всегда, а в бурный день
    // к ней добавляются все настоящие выбросы.
    ...
    <div>
      ...
      <div data-tour="screener-toolbar" className="flex flex-wrap items-center mb-4 md:mb-6 gap-2 md:gap-3">
        ...
        {/* Горизонт: дневная лента / недельная (14 дней). Стоит сразу за
            ...
            сам расчёт, а не состав строк. «?» живёт трейлингом ВНУТРИ пилюли:
