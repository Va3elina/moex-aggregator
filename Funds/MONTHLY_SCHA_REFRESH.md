...
3. Я проверяю: assets>0, Σвесов≈100%, спот-чек значений → пишу в БД.
   **Обязательный спот-чек после импорта** (баги парсера, детали в скилле
   `moex-fund-scha-backfill` «Подводные камни»):
   - implied-price `amount/positions` в разумном диапазоне (не «триллион штук» = cid:9
     Т-Кап; не «positions<1000» у US/JE/GB/NL-isin = код страны вместо количества);
   - ВИМ-PDF → прогнать детектор ×1000 (ниже);
   - сплит (кол-во ×R, цена ÷R, стоимость≈const) — данные корректны, адъюст делает фронт/`/snapshot`.
