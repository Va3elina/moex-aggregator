...
def _find_metadata(df: pd.DataFrame) -> dict:
    ...
    for i in range(min(50, len(df))):
        ...
        if not meta["snapshot_date"]:
            ...
            # ⚠️ На многолистовых XBRL-экспортах (Атон и др.) «сопроводительный» лист
            # содержит номер/дату РЕГИСТРАЦИИ ЛИЦЕНЗИИ УК в том же ISO-формате
            # (напр. «21-000-1-00047 2001-02-28») — это НЕ отчётная дата, но сканируется
            # раньше настоящего листа формы. Настоящая дата всегда идёт сразу после слова
            # «Значения» (заголовок Раздела 2 «Параметры справки» — регламентированный
            # текст формы, единый для всех УК; в отличие от кода валюты, который разные
            # УК форматируют по-разному — «643-RUB» или голое «643» — «Значения» надёжнее).
            # Без этого якоря ложная лицензионная дата залипает первой и «замораживает»
            # snapshot_date на неверном значении.
            m_iso = re.search(r"\b(\d{4})[.-](\d{2})[.-](\d{2})\b", joined)
            if m_iso and "Значен" in joined:
