...
COL_PATTERNS = {
    ...
    # «Сведения, позволяющие установить ценные бумаги» — ISIN-колонка подраздела «иные ценные
    # бумаги» без слова ISIN в шапке (облигации ВЭБ.РФ у Флагманского: 4 % активов терялись).
    "isin": re.compile(r"ISIN|позволяющие\s+установить\s+ценн", re.I),
            ...
            if value_rub > 0:
                for cand in reversed(nums[:-1]):
                    if cand <= 0 or cand != int(cand):
                        continue
                    if int(cand) in _ISO_COUNTRY_CODES:
                        continue
                    # Потолок 500k ₽/шт: доспличные дорогие бумаги (Транснефть до
                    # сплита 1:100 ~105 000 ₽, Норникель и т.п.) — это реальные цены.
