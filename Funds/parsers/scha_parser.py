...
# Строка дебиторской задолженности по бумаге: «Погашение купона по облигации, сумма к
# получению», у Т-Капитала «Погашени е облигации, сумма к погашени ю» (перенос внутри слова).
RECEIVABLE_ROW_PATTERN = re.compile(r"сумма\s*к\s*(получ|погаш)")

# ISO 3166-1 numeric коды стран. В подразделе «Акции иностранных эмитентов»
# формы 0420502 стоят коды страны депозитария (840=США/BoNY) и эмитента
# (196=Кипр, 528=Нидерланды и т.п.) — их НЕЛЬЗЯ принять за количество бумаг.
# Набор сфокусирован на странах ГДР/АДР росскомпаний + домицилей депозитариев.
_ISO_COUNTRY_CODES = {36, 56, 124, 196, 250, 276, 372, 398, 442, 528, 643, 756, 784, 826, 832, 840,
                      # офшорные домицили эмитентов расписок: 60 Бермуды, 92 Британские
                      # Виргинские о-ва (VK/Mail.ru, Fix Price, Лента), 136 Каймановы о-ва,
                      # 470 Мальта, 831 Гернси, 833 о. Мэн. С потолком цены 100 млн ₽/шт
                      # код 92 давал «правдоподобные» 1,1 млн ₽ за расписку VK (регрессия
                      # на справках SBMX/OPIF-1003 2023 года, поймана прогоном 1452 PDF).
                      60, 92, 136, 470, 831, 833}

# ИНН/ОГРН эмитента стоят в строке формы ЛЕВЕЕ количества. Парсер доходит до них, когда
# настоящее количество отброшено (30.04.2026: у выпусков номиналом 1–10 млн ₽ в количестве
# оказались ИНН ВТБ 7702070139, Сбера 7707083893, РЖД 7708503727). Узнаём по контрольной
# сумме: ИНН юрлица — 10 цифр, ОГРН — 13. Числа, кратные 1000, не трогаем: так выглядят
# реальные крупные позиции (ВТБ ао до обратного сплита — 8 797 190 000 шт.), а контрольная
# сумма случайно сходится у каждого десятого числа.
_INN_WEIGHTS = (2, 4, 10, 3, 5, 9, 4, 6, 8)


def looks_like_issuer_code(n) -> bool:
    """True если целое n — ИНН юрлица или ОГРН по контрольной сумме и не кратно 1000."""
    if n is None or n != int(n) or int(n) % 1000 == 0:
        return False
    s = str(int(n))
    if len(s) == 10:
        return sum(w * int(c) for w, c in zip(_INN_WEIGHTS, s)) % 11 % 10 == int(s[9])
    if len(s) == 13:
    ...
    for table in tables:
        ...
        for row in table:
            ...
            if "репо" in row_text or "сделк" in row_text:
                continue
            # Дебиторская задолженность по бумаге («Погашение купона/тела облигации,
            # сумма к получению», у Т-Капитала «сумма к погашению»): ISIN — ссылка, в
            # строке номер п/п, ИНН/ОГРН и сумма. Не позиция: иначе «ЯТЭК 1P-1 —
            # 6 000 000 шт. по 1 ₽» (OBLG 31.10.2023) или номер строки за количество.
            if RECEIVABLE_ROW_PATTERN.search(row_text):
                continue
            ...
            value_rub = nums[-1]
            # Количество: первое СПРАВА число (перед стоимостью), НЕ являющееся
            # кодом страны ISO и дающее вменяемую цену value/qty.
            # У ОТЕЧЕСТВЕННЫХ бумаг это ближайшее слева число → без изменений.
            # У ИНОСТРАННЫХ эмитентов в строке стоят ДВА кода страны — депозитария
            # (840=США, BoNY) и эмитента (196=Кипр у ГДР FIX PRICE/ROS AGRO/CIAN/
            # HH/TCS/GLTR; 528=Нидерланды у Яндекс/Озон). Старое nums[-2] хватало
            # код эмитента → «196 расписок». Цена value/код иногда «правдоподобна»
            # (Т-Капитал: 1.46M/196=7457₽), поэтому одной цены мало — исключаем коды.
            positions = nums[-2]  # дефолт (fallback)
            if value_rub > 0:
                for cand in reversed(nums[:-1]):
                    if cand <= 0 or cand != int(cand):
                        continue
                    if int(cand) in _ISO_COUNTRY_CODES or looks_like_issuer_code(cand):
                        continue
                    # Потолок 100 млн ₽/шт: доспличные дорогие акции (Транснефть до
                    # сплита 1:100 ~105 000 ₽) и облигации с крупным номиналом —
                    # 1 млн ₽ (РЖД 1Б-06) и 10 млн ₽ (субординированные ВТБ/Сбера, ~10,8 млн
                    # с НКД). Прежний потолок 500 000 ₽ отбрасывал их настоящее количество,
                    # цикл уходил влево и брал ИНН эмитента: 30.04.2026 у OBLG «ВТБ Т2-3 —
                    # 7 702 070 139 шт.» по 0,10 ₽ вместо 74 шт. (миграция 094).
                    if 0.01 <= value_rub / cand <= 100_000_000:
                        positions = cand
                        break
