...
def _parse_assets_from_tables_rowwise(tables: list, isin_pif: Optional[str] = None) -> list[dict]:
    ...
    for table in tables:
        ...
        for row in table:
            ...
            value_rub = nums[-1]
            # Количество: первое СПРАВА число (перед стоимостью), дающее вменяемую
            # цену value/qty. У ОТЕЧЕСТВЕННЫХ бумаг это ближайшее слева число —
            # поведение не меняется. У ИНОСТРАННЫХ эмитентов между количеством и
            # стоимостью вклинивается КОД СТРАНЫ ISO (196=Кипр у ГДР ROS AGRO PLC,
            # 840=США у АДР) → старое nums[-2] хватало его → «196 расписок». Цена
            # value/код_страны заведомо абсурдна (>100k ₽) и отсекается.
            positions = nums[-2]  # дефолт (старое поведение как fallback)
            if value_rub > 0:
                for cand in reversed(nums[:-1]):
                    if cand > 0 and cand == int(cand) and 0.01 <= value_rub / cand <= 100_000:
                        positions = cand
                        break
