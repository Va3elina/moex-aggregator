...
SCRIPTS = {
    ...
    # ⚠️ ПЛАТНЫЙ ИСТОЧНИК С СУТОЧНОЙ КВОТОЙ (400 запросов, обнуляется каждый день;
    # здесь было написано «месячной» — я ошибся при первом заходе, и по этой ошибке
    # чуть не построил помесячную раскладку расхода). Скрипт сам считает расход в
    # api_budget и берёт за прогон только свою долю остатка, поэтому его МОЖНО
    # дёргать ежедневно: он не «обходит всех», а докачивает самых давних.
...
class MainOrchestrator:
    ...
    async def run_forever(self):
        ...
        if is_trading:
            ...
            # ⚠️ ОБЯЗАТЕЛЬНО И ЗДЕСЬ. Через 24 строки этот блок ставит
            # last_daily_update = сегодня, и блок 19:10 на сегодня уже не сработает.
            # Значит любая задача, которой тут нет, пропускает весь день, если
            # оркестратор перезапускался до 19:10 — а он перезапускается на каждом
            # деплое. fm_cards_daily так и потерял свой первый день: он стоял только
            # в блоке 19:10, и увидеть это можно было лишь по ОТСУТСТВИЮ строки в
            # pipeline_runs, на которое никакая проверка не смотрит.
