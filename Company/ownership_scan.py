...
# Квазиказначейские пакеты: компания владеет собственными акциями через дочерние
# структуры. Это НЕ ребро между эмитентами — обе стороны одна и та же компания, — но
# факт содержательный: у Магнита так лежит 33%, у Сургутнефтегаза 70%. Такой пакет
# не голосует и не торгуется, то есть реальный free float меньше, чем кажется.
TREASURY_MARK = "казначейск"          # ловит и «казначейские», и «квазиказначейские»
# Держатели, которых по названию не опознать, но проверено вручную, что это оно же.
TREASURY_EXTRA = {
    ("IRAO", "АО «Интер РАО Капитал»"),
    ("SNGS", "Дочерние структуры Сургутнефтегаза"),
    ("SMLT", "Самолет-резерв"),
}
...
def main():
    ...
    result = {"собрано": datetime.now().isoformat(timespec="seconds"),
              "рёбра": [], "казначейские": [], "неоднозначно": [], "не_эмитенты": [],
              "без_страницы": []}
    for i, t in enumerate(tickers, 1):
        ...
        for h in holders:
            rec = {"эмитент": t, "владелец": h["holder"], "доля": h["share_pct"],
                   "структура_от": as_of}
            # Казначейство проверяем ПЕРВЫМ: «ООО "ФСК-Управление активами" —
            # квазиказначейские» иначе уехало бы в рёбра как ФСК-Россети сама к себе.
            if TREASURY_MARK in h["holder"].lower() or (t, h["holder"]) in TREASURY_EXTRA:
                result["казначейские"].append(rec)
                continue
    ...
    log.info("ИТОГ: рёбер=%d казначейских=%d (у %d компаний) неоднозначных=%d "
             "не-эмитентов=%d без страницы=%d",
             len(result["рёбра"]), len(result["казначейские"]),
             len({r["эмитент"] for r in result["казначейские"]}),
