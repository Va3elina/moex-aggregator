...
# ── ГЛАВНАЯ ПРОВЕРКА (см. Поправку №1 в PREREGISTRATION.md) ──────────────
# Берём K лучших ПО BASELINE и сравниваем их форвардный результат со
# случайными наборами того же размера в том же периоде. Случайный набор —
# это «что дала бы бумага сама по себе», поэтому разница очищена от дрейфа.
K, DRAWS = SPEC["top_k"], SPEC["placebo_draws"]
rng = np.random.default_rng(1)
print(f"ГЛАВНОЕ: топ-{K} по истории против случайных {K} вперёд")
print(f"  {'бумага':<8}{'отобранные':>13}{'случайные':>12}{'надбавка':>11}{'перцентиль':>12}")
adv = []
for st, g in m.groupby("sectype_b"):
    ok = g.dropna(subset=["sr_b", "mean_f"])
    if len(ok) < 200:
        continue
    sel = ok.nlargest(K, "sr_b")["mean_f"].mean()
    pool = ok["mean_f"].values
    null = np.array([rng.choice(pool, K, replace=False).mean() for _ in range(DRAWS)])
    pct = 100 * (null < sel).mean()
    adv.append(sel - null.mean())
    print(f"  {st:<8}{sel*100:>+12.3f}%{null.mean()*100:>+11.3f}%"
          f"{(sel-null.mean())*100:>+11.3f}{pct:>12.1f}")

if adv:
    a = np.array(adv)
    sign_p = stats.binomtest(int((a > 0).sum()), len(a)).pvalue
    print(f"\n  надбавка отбора: средняя {a.mean()*100:+.3f} п.п., "
          f"плюс у {(a > 0).sum()} из {len(a)}, знаковый тест p={sign_p:.3f}")
    print(f"  ориентир с истории: +0.172 п.п., плюс у 9 из 9")

# справочно: ранговая связь (была главным критерием до Поправки №1)
ok = m.dropna(subset=["sr_b", "sr_f"])
rho, p = stats.spearmanr(ok.sr_b, ok.sr_f)
print(f"\n  справочно, ранговая связь по всей сетке: {rho:+.3f} (p={p:.4f}) — "
      f"засорена волатильностью, решающей не считать")
