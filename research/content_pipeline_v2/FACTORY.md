...
С 19.09 карточка — как пост в канале: график (находки/связки) отдельным фото сверху, под ним одно
сообщение — САМ ПОСТ, дальше два свёрнутых блока (`<blockquote expandable>`): «🔎 откуда пост» (выжимка
`signals/brief_summary.py` / текст новости, «почему взяли») и «судья» (вердикт, правка, сомнения).
Отдельных сообщений «Контекст» больше нет. Лимит 4096 видимых символов: сначала режется «откуда», потом
сомнения (`_card_view`). Получатели — админ и коллега (`CONTENT_DRAFT_EXTRA_CHAT_IDS`); кнопки: Одобрить
(публикует в @FrameTool), Править (`fire_revision`), Отклонить (код причины из `config.REVIEW_REASON_CODES`).
