"""Merge the two parallel-fetched SBER chain files, dedup by (TRADEDATE, SECID).

P1 covers 2020-01 → 2024-05, P2 covers 2024-01 → 2026-05.
Overlap region 2024-01 → 2024-05 will have duplicates — keep first.

Output:
  sber_options_history_merged.csv
  sber_futures_history_merged.csv
"""
import pandas as pd
...
def main():
    ...
    opt = merge(['sber_options_history.csv', 'sber_options_history_part2.csv'],
                ['TRADEDATE', 'SECID'], 'sber_options_history_merged.csv')

    print("\nMerging futures:")
    fut = merge(['sber_futures_history.csv', 'sber_futures_history_part2.csv'],
                ['TRADEDATE', 'SECID'], 'sber_futures_history_merged.csv')
