"""Backtest GAZP LONG strategy on FIZ (retail) contrarian signal.
...
Strategy: LONG GAZP when FIZ_LSR < threshold_low, exit after fixed hold OR when FIZ_LSR > threshold_high.
...
def make_df():
    df = gazp[['close']].join(futoi[['fiz_long', 'fiz_short', 'yur_long', 'yur_short']], how='inner')
