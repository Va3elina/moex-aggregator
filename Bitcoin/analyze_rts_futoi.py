"""Analyze RTS futoi for smart money signals + build rts_futoi_signals.csv.

Mirror of analyze_gazp_futoi.py. YUR/FIZ are zero-sum on FORTS clearing.
Key test: which direction does FIZ-LSR predict on RTS? (retail contrarian vs trend)
"""
import pandas as pd
import numpy as np
...
futoi = pd.read_csv(ROOT/'rts_futoi_history.csv', parse_dates=['date']).set_index('date').sort_index()
...
print(f"RTS:   {rts.index[0].date()} -> {rts.index[-1].date()}, {len(rts)} rows\n")
...
g_close = rts[['close']].rename(columns={'close': 'rts_close'})
merged = df.join(g_close, how='inner')
for h in [5, 10, 20, 60]:
    merged[f'fwd_{h}d'] = merged['rts_close'].pct_change(h).shift(-h) * 100

print("\n" + "=" * 90)
print("FORWARD RETURNS by FIZ LSR quintile (retail):")
print("=" * 90)
...
out = ROOT / 'rts_futoi_signals.csv'
df.to_csv(out)
