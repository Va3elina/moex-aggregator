"""Master builder: ALL options-derived indicators for RTS, daily 2020-2026.
...
Output: rts_option_indicators_daily.csv with columns:
...
from parse_ri_secid import parse_secid, resolve_year
...
CONTRACT_MULT = 1  # RTS option = 1 futures (index points x100)


def load_chain():
    p = ROOT / 'rts_options_history_merged.csv'
    df = pd.read_csv(p)
...
def load_expiry_map():
    m = pd.read_csv(ROOT / 'rts_expiry_map.csv')
...
def build():
    ...
    res.to_csv(ROOT / 'rts_option_indicators_daily.csv', index=False)
