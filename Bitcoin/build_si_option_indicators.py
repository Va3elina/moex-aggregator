"""Master builder: ALL options-derived indicators for Si, daily 2020-2026.
...
Output: si_option_indicators_daily.csv with columns:
...
from parse_si_secid import parse_secid, resolve_year
...
CONTRACT_MULT = 100  # Si option = 100 shares


def load_chain():
    p = ROOT / 'si_options_history_merged.csv'
    df = pd.read_csv(p)
...
def load_expiry_map():
    m = pd.read_csv(ROOT / 'si_expiry_map.csv')
...
def build():
    ...
    res.to_csv(ROOT / 'si_option_indicators_daily.csv', index=False)
