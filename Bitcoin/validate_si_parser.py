"""Validate Si SECID parser against MOEX securities API on N samples.
...
from parse_si_secid import parse_secid
...
def get_chain(date):
    ...
    while True:
        r = requests.get('https://iss.moex.com/iss/history/engines/futures/markets/options/securities.json',
                         params={'date': date, 'assetcode': 'Si', 'iss.meta': 'off', 'start': start},
...
def main():
    secids = set()
    for date in ['2026-05-15', '2024-06-03', '2022-03-10', '2021-09-15', '2020-09-15']:
        try:
    ...
    sample = secids[:N]
    print(f"Validating {len(sample)} Si SECIDs against API...")
