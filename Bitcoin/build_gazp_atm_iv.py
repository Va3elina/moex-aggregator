"""Build daily ATM IV series for GAZP from option settlement prices.
...
def load_chain():
    ...
    for fname in ['gazp_options_history_merged.csv', 'gazp_options_history.csv']:
        p = ROOT / fname
        if p.exists():
...
def build():
    ...
    out.to_csv(ROOT / 'gazp_atm_iv_daily.csv', index=False)
