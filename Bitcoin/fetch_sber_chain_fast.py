"""Fast SBER options chain history fetcher.

Downloads daily snapshot of all SBRF options 2020-2026 with progress checkpointing.
Pagination limit hard-set to 100 by MOEX, so we paginate.

Output: sber_options_history.csv (settlement prices, OI, volume per option per day)
        sber_futures_history.csv (underlying SBRF futures settles per day)
"""
import requests
import csv
import time
...
OUT_OPT = ROOT / "sber_options_history.csv"
OUT_FUT = ROOT / "sber_futures_history.csv"
PROG = ROOT / ".sber_chain_progress.json"
...
def fetch_day(d, url):
    ...
    cols = None
    while True:
        params = {'date': d, 'assetcode': 'SBRF', 'iss.meta': 'off', 'start': start}
...
def main():
    ...
    # Parse SBER option SECID to extract strike + expiry + type
