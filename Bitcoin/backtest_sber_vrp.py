"""Backtest SBER VRP (per-asset, options-derived) strategies.

VRP = ATM_IV (from SBER options via Black-76) - RV30 (SBER stock realized vol).
...
def load():
    iv = pd.read_csv(ROOT/'sber_atm_iv_daily.csv', parse_dates=['date']).set_index('date').sort_index()
