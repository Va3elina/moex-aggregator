"""Fast Si futoi EOD fetcher — only last snapshot per day via latest=1."""
import requests
...
OUT = Path(__file__).parent / "si_futoi_history.csv"
URL = "https://iss.moex.com/iss/analyticalproducts/futoi/securities/Si.json"
...
def main():
    ...
    with open(OUT, 'w', newline='') as f:
        ...
        w.writerow(['date', 'yur_long', 'yur_short', 'fiz_long', 'fiz_short',
                    'yur_net', 'fiz_net', 'yur_total_n', 'fiz_total_n',
                    'smart_money_ratio', 'fiz_lsr', 'yur_lsr'])
        for date_key in sorted(by_date.keys()):
            ...
            # fiz_lsr = fiz long / abs(fiz short): >1 means fiz net long USD (ruble-bearish)
            fiz_lsr = fiz_long / (fiz_short + 1)
            yur_lsr = yur_long / (yur_short + 1)
            w.writerow([date_key, yur_long, yur_short, fiz_long, fiz_short,
                        yur_net, fiz_net, yur_n, fiz_n, round(smart_ratio, 4),
                        round(fiz_lsr, 4), round(yur_lsr, 4)])
