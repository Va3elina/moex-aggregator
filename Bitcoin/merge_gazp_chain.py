"""Merge the two parallel-fetched GAZP chain files, dedup by (TRADEDATE, SECID).
...
Output:
  ...
  gazp_futures_history_merged.csv
"""
import pandas as pd
...
def main():
    ...
    opt = merge(['gazp_options_history.csv', 'gazp_options_history_part2.csv'],
                ['TRADEDATE', 'SECID'], 'gazp_options_history_merged.csv')

    print("\nMerging futures:")
    fut = merge(['gazp_futures_history.csv', 'gazp_futures_history_part2.csv'],
                ['TRADEDATE', 'SECID'], 'gazp_futures_history_merged.csv')
