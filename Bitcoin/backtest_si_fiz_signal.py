"""Backtest Si LONG strategy on FIZ (retail) contrarian signal.
...
Strategy: LONG Si when FIZ_LSR < threshold_low, exit after fixed hold OR when FIZ_LSR > threshold_high.
...
sber = pd.read_csv(ROOT/'si_daily.csv', parse_dates=['date']).set_index('date').sort_index()
# Use rub_per_usd as the price (raw 'close' points = rub_per_usd*1000; returns identical)
sber['close'] = sber['rub_per_usd']
futoi = pd.read_csv(ROOT/'si_futoi_history.csv', parse_dates=['date']).set_index('date').sort_index()
