...
strategy("MultiFactor BTC Strategy (F1 Trend + F2 Season + F4 Flow + F6 Vol)",
     overlay=true,
     initial_capital=100,
...
// ============ Параметры ============
thr_entry = input.float(0.5,  "Score threshold to ENTER long", step=0.05)
...
w_F4      = input.float(1.0,  "Weight F4 (flow)",  step=0.1)
...
get_F2() =>
    ...
    months_since >= 6  and months_since <= 18 ?  1.0 :
       months_since > 18 and months_since <= 24 ?  0.5 :
       months_since > 24 and months_since <= 30 ?  0.0 :
       months_since > 30 and months_since <= 42 ? -0.5 :  0.3
...
composite = total_weight > 0 ?
    (f1_score * w_F1 + f2_score * w_F2 + f4_score * w_F4 + f6_score * w_F6) / total_weight : 0.0
