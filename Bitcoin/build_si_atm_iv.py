"""Build daily ATM IV series for Si from option settlement prices.
...
def load_chain():
    ...
    for fname in ['si_options_history_merged.csv', 'si_options_history.csv']:
        p = ROOT / fname
        if p.exists():
...
def build():
    ...
    out.to_csv(ROOT / 'si_atm_iv_daily.csv', index=False)
