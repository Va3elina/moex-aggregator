"""Validate GAZP SECID parser against MOEX securities API on N samples.

Pulls a chain snapshot, parses each SECID locally, then fetches the official
STRIKE + OPTIONTYPE from /iss/securities/{SECID}.json and compares.
Target: 100% match (as achieved for SBER).
"""
...
def get_chain(date):
    ...
    while True:
        r = requests.get('https://iss.moex.com/iss/history/engines/futures/markets/options/securities.json',
                         params={'date': date, 'assetcode': 'GAZR', 'iss.meta': 'off', 'start': start},
...
def main():
    # gather a diverse set of SECIDs across a few dates
    secids = set()
    for date in ['2026-05-15', '2024-06-03', '2022-03-10', '2021-09-15']:
        try:
    ...
    sample = secids[:N]
    print(f"Validating {len(sample)} GAZP SECIDs against API...")
