"""Master builder: ALL options-derived indicators for SBER, daily 2020-2026.
...
Output: sber_option_indicators_daily.csv with columns:
...
CONTRACT_MULT = 100  # SBER option = 100 shares


def load_chain():
    p = ROOT / 'sber_options_history_merged.csv'
    df = pd.read_csv(p)
...
def load_expiry_map():
    m = pd.read_csv(ROOT / 'sber_expiry_map.csv')
...
def build():
    ...
    res.to_csv(ROOT / 'sber_option_indicators_daily.csv', index=False)
