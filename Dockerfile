...
# BRAIN_WARMUP=0 (06.09.2026, решение Вадима): модель НЕ греется заранее. Замер за
# 30 дней: смысловой поиск сработал 1 раз, «похожие» — 0, остальные 47 обращений к
# мозгу — SQL без модели. Держать 810 МБ ради этого незачем: модель подхватит только
# тот воркер, которому пришёл смысловой запрос (первый ответ ~11 с — это фоновый шаг
# конвейера, не страница), а recycle по max-requests память вернёт. Вернуть прогрев —
# BRAIN_WARMUP=sync (с --preload) или 1 (без него).
ENV BRAIN_WARMUP=0
