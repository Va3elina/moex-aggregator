---
name: moex-billing
description: Биллинг Фрейма — T-Bank эквайринг (прод) + миграция на ЮKassa (тест-фаза). Use when user says «оплата», «биллинг», «юкасса», «yookassa», «тбанк», «рекурренты», «подписка не оплатилась», «включи тест юкассы», «проверь включили ли автоплатежи», «возврат денег», «отвяжи карту», or any payment/subscription ops task. Содержит ключи (см. references/secrets.local.md), архитектуру кода, рунбуки проб/включения теста/отката, gotchas.
---

# Биллинг Фрейма: T-Bank (прод) + ЮKassa (миграция, тест-фаза)
...
- **ЮKassa: тест-фаза.** Магазин 1398109 (ИП Тория), боевой, фискализация yoo_receipt ВКЛ.
  Методы: bank_card, sbp, sberbank (SberPay), tinkoff_bank (T-Pay), yoo_money.
- 🔴 **Рекурренты ЮKassa ещё НЕ включены** — заявка создана 02.07 в чате ЛК (Александр),
  срок 1–3 раб. дня. Порядок от менеджера: сначала КАРТЫ → потом ВТОРОЙ запрос на
  SberPay/T-Pay/СБП. Alfa Pay автоплатежи не поддерживает вообще (не проси).
- **Код готов и спит** (PR #309): активация = env-переменные, см. рунбук ниже.
