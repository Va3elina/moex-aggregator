...
- **🐞 Иностранные ГДР/АДР — код страны вместо количества**: в подразделе «Акции иностранных
  ...
  `_ISO_COUNTRY_CODES`, количество = первое СПРАВА число с вменяемой ценой (потолок **100 млн ₽/шт**,
  см. ниже). **Спот-чек:** `positions<1000` у US/JE/GB/NL-isin = недобитый код.
- **🐞 ИНН эмитента вместо количества у облигаций с крупным номиналом** (10.09.2026, справки ВИМ
  на 30.04.2026, OBLG/OPIF-54, миграция 094): у субординированных ВТБ/Сбера (номинал 10 млн ₽) и
  РЖД 1Б-06 (1 млн ₽) цена за штуку выше прежнего потолка rowwise 500k ₽ → настоящее количество
  (74) отбрасывалось, цикл уходил влево и брал **ИНН** («ВТБ Т2-3 — 7 702 070 139 шт.» по 0,10 ₽).
  Стоимость/доля верные, `is_implausible_row` (порог 0,0001 ₽) не ловил. Фикс: потолок 100 млн ₽/шт
  + `looks_like_issuer_code` (контрольная сумма ИНН-10/ОГРН-13, не кратно 1000) исключает кандидата;
  в `save_assets` такое количество пишется NULL с warning. ⚠️ Кратность 1000 обязательна: у реальных
  позиций ВТБ ао до обратного сплита (8 797 190 000 шт.) контрольная сумма ИНН сходится случайно.
  **Спот-чек после заливки:** `amount_rub/positions` у облигаций ≈ номинал (ISS `FACEVALUE`), а
  `positions ≥ 1e9` при цене < 1 ₽ — почти наверняка ИНН. Чинить: штуки = стоимость / цена за шт.
- **🐞 Дебиторка по бумаге за позицию** (rowwise): строки «Погашение купона/тела облигации, сумма
  к получению» (у Т-Капитала «сумма к погашению») несут ISIN бумаги-ссылки, номер п/п, ИНН/ОГРН и
  сумму → парсер делал из них «ЯТЭК 1P-1 — 6 000 000 шт. по 1 ₽» (OBLG 31.10.2023) или брал номер
  строки за количество. Теперь пропускаются по `RECEIVABLE_ROW_PATTERN`, как РЕПО. **Спот-чек:**
  `positions ≈ amount_rub` (цена ровно 1 ₽) — почти всегда дебиторка.
- **🐞 VK ГДР (US5603172082) — количество не достаётся позиционно**: в строке подряд количество
  161 620, регномер эмитента в BVI 655058 и код BVI 092 → rowwise берёт 655058 (SBMX/OPIF-1003
  2023: «цена» 107–186 ₽ при рыночной ~650 ₽), в TMOS в БД лежит 92. Код 92 и прочие офшоры
  (60/136/470/831/833) теперь в `_ISO_COUNTRY_CODES`. Чинить руками reconstruct-приёмом (ниже).
- **Проверка правки парсера** — прогон старой и новой версии `parse_scha` по всем PDF в
  `~/Downloads` (scha_manual, SCHA_исходники, scha_alfa_clean, funds_organized; 1452 файла) с diff по
  ISIN. Так пойманы регрессии потолка 100 млн (код BVI у VK, строки дебиторки).
- **🐞 Двойная заливка среза** (OPIF-54 30.04.2026): повторный импорт без `--replace` с другим
  написанием имени (юр-имя ↔ короткое MOEX) не матчит ON CONFLICT (ключ содержит asset_name) →
  обе копии в срезе, Σдолей ~200 %. **Спот-чек:** `(fund, date, isin, amount_rub)` в ≥2 строках.
  Перезаливать срез — только `--replace`. Справка interfax_manual замещает vim_sdr того же месяца
  всегда (миграция 093).
