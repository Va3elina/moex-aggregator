...
4. `⚠️ ВАЖНО: используй ТОЛЬКО ASCII-домен (punycode) в URL, НЕ кириллический
   таймфрейм.рф — он ломает запрос в некоторых окружениях (проверено).`
5. **Явный `curl` через bash**, не абстрактное "сделай PATCH" — модель иначе может
   ...
   ```
   curl -s -X PATCH https://xn--80aklbnczmv.xn--p1ai/api/internal/content-news/CANDIDATE_ID/<endpoint> \
