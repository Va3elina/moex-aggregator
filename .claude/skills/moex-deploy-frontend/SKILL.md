...
**Главное правило** (с 2026-06-09): **попало в `main` → CI задеплоил.**
> Дефолт — **НЕ прямой push в `main`, а ветка → PR → мёрж** (мёрж и есть тот push,
> что триггерит CI). См. `CONTRIBUTING.md` / `moex-git-workflow`. Прямой
> `git push origin main` — только solo-исключение по явной просьбе.

Когда юзер говорит «задеплой фронт», после typecheck + commit это **мёрж PR в
`main`** (или прямой push, если так решили) и **наблюдаешь за CI**, а не лезешь на сервер.
