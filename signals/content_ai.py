...
BRIEF_VERSION = 24   # v24: в блоке мозга — входы/выходы из индексов и события фондов
...
def _brain_block(db, row, ctxs=None) -> dict:
    ...
    for tk, c in ctxs.items():
        ...
        # Официальные события компании — все, не только ставшие кандидатами (Вадим 10.09).
        if (c.get("раскрытия") or {}).get("всего"):
            блок[f"раскрытия_компании_за_{BRAIN_CONTEXT_DAYS}_дней"] = (
                f"{c['раскрытия']['всего']} [B, FinanceMarker]; последние: " +
                " | ".join(f"{(э.get('время') or '')[:10]} {э['заголовок'][:90]}" for э in c["раскрытия"]["элементы"][:3]))
        if (c.get("объявления_биржи") or {}).get("всего"):
            блок[f"объявления_биржи_за_{BRAIN_CONTEXT_DAYS}_дней"] = (
                f"{c['объявления_биржи']['всего']}; последние: " +
                " | ".join(f"{(э.get('время') or '')[:10]} {э['заголовок'][:90]} [{э.get('уровень') or '?'}]"
                           for э in c["объявления_биржи"]["элементы"][:3]))
        if (c.get("отчёты") or {}).get("элементы"):
            блок["отчёты_с_цифрами"] = [
                (f"{э['заголовок']} [A, скачан {(э.get('время') or '')[:10]}]: "
                 + (э.get("сводка") or "цифры ещё не извлечены"))[:400]
                for э in c["отчёты"]["элементы"][:2]]
        if (c.get("индексы_события") or {}).get("элементы"):
            блок["входы_и_выходы_из_индексов"] = [
                f"{(э.get('время') or '')[:10]} {э['заголовок']} [A]" for э in c["индексы_события"]["элементы"][:3]]
        if (c.get("фонды_события") or {}).get("всего"):
            блок[f"фонды_меняли_позицию_за_{BRAIN_CONTEXT_DAYS}_дней"] = (
                f"{c['фонды_события']['всего']} [A, раскрытия УК — месячные срезы, не сделки]; последние: " +
                " | ".join(f"{(э.get('время') or '')[:10]} {э['заголовок'][:90]}" for э in c["фонды_события"]["элементы"][:3]))
        out[tk] = блок
    if out:
