...
def _related_context(db, tickers, as_of) -> dict:
    ...
    out["ПОЯСНЕНИЕ"] = (
        ...
        "ОБЪЁМ: это ФОН, а не содержание поста. Из каждой компании — самый крупный "
        "факт, ОДНИМ КОРОТКИМ ПРЕДЛОЖЕНИЕМ. Не втискивать две компании в одно "
        "предложение и не добавлять уточнений в скобках. "
        # ⚠️ Порядок внутри абзаца задан замечанием Вадима 01.09: из двух вариантов
        # он выбрал тот, где СНАЧАЛА сказано, кто кому кем является, и только потом
        # что у кого произошло. Без этого читатель встречает «Сегежа получила рейтинг
        # ruB» до того, как узнал, при чём тут Сегежа.
        "ПОРЯДОК: сначала одной фразой КТО КОМУ КЕМ ЯВЛЯЕТСЯ («Сегежа входит в группу "
        "Системы, а сама Система — крупный акционер Озона»), и только потом что у "
        "кого произошло. "
