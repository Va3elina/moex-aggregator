...
def _position_phrases(asset_id: str, clgroup: str | None, as_of=None) -> dict:
    ...
    return {
        ...
        # ⚠️ Имя поля переименовано вслед за содержанием: «доля_..._в_ои» тянула
        # обратно к процентам от ОИ, которых канал не пишет ни разу на 1459 постов.
        "размер_позиции_против_пика": _size_vs_peak(word, share, pcts, last,
                                                     _window_ru(span)),
...
def _related_context(db, tickers, as_of) -> dict:
    ...
    out["ПОЯСНЕНИЕ"] = (
        ...
        "ОБЪЁМ: это ФОН, а не содержание поста. По ОДНОЙ короткой фразе на компанию "
        "(их тут максимум две), и обе — в одном абзаце. Отдельного абзаца на каждую "
        "компанию не делать, перечислять все пункты подряд не надо: из каждой "
        "компании бери самый крупный факт."
    )
    return out
...
def _size_vs_peak(word: str, share: float, pcts, last, window: str) -> str:
    """Размер позиции — СРАВНЕНИЕМ С ПИКОМ, без процентов от открытого интереса.

    ⚠️ Вадим 01.09: «Чистая позиция физлиц сейчас — 5% открытого интереса» — «вот эта
    формулировка процентов от ОИ очень не понятна и странная, так вообще не пишем».
    Замер по корпусу подтвердил дословно: «% открытого интереса» — 0 постов из 1459,
    «доля … открытого интереса» — 0, само словосочетание «открытый интерес» — 6
    постов, и в единственном жанровом примере это НАЗВАНИЕ нашего индикатора, а не
    описание доли.

    Как канал говорит про размер на самом деле: «наращивают чистый объём покупок,
    который потихоньку приближается к пикам 2021 г.» и «самый густонаселённый трейд
    среди всех бумаг». То есть через сравнение с пиком и через место среди бумаг —
    никогда через процент от ОИ.

    Поэтому информацию («позиция тонкая» / «позиция у максимума») оставляем, а
    единицу измерения выбрасываем. Доля считается по-прежнему — она нужна, чтобы
    вычислить кратность, — но в бриф уходит уже сравнением.

    ⚠️ Пик берётся в ТОМ ЖЕ направлении: сравнивать лонг с пиковым шортом нельзя,
    это разные состояния толпы, а не больше/меньше одного и того же.
    """
    same_dir = [abs(x) for x in pcts if (x > 0) == (last > 0)]
    if not same_dir or share < 1:
        return f"чистый {word}"
    peak = max(same_dir)
    t = _times(peak / share)
    if not t:
        return f"чистый {word} — у своего максимума за {window}"
    return f"чистый {word} сейчас {t} меньше, чем на пике за {window}"
...
def _rating_history(db, headline: str, raw_text: str, tickers, as_of) -> dict:
    ...
    out["ПОЯСНЕНИЕ"] = (
        ...
        "называть нельзя. "
        "ОБЪЁМ: это ФОН. Прежний уровень — одной фразой в том же абзаце, где само "
        "решение агентства. Отдельного абзаца под историю рейтингов не делать."
    )
    return out
