...
BRIEF_VERSION = 18   # v18: блок чисел снова живой — коды и единицы FinanceMarker
...
def _company_fundamentals(db, asset_id: str, tickers, event_type: str, as_of,
    ...
    if out:
        ...
        # ⚠️ ОДНО ЧИСЛО МОЖНО — НО ТОЛЬКО КОГДА ОНО СПОРИТ С ПОВОДОМ ИЛИ ЕГО
        # ОБЪЯСНЯЕТ. Полный запрет («числа не переносить») оказался слишком жёстким, и
        # проверка это показала на кандидате 1638: АКРА понизило рейтинг АФК Системы,
        # а Долг/EBITDA у неё за год УПАЛ с 4,68 до 3,46 и убыток сократился. «Рейтинг
        # режут компании, у которой долг стал легче» — это сюжет, а не фон, и без
        # цифры он не рассказывается.
        #
        # Отличие от прежнего «максимум одно число, если объясняет повод» не в
        # количестве, а в КРИТЕРИИ: раньше годилось любое относящееся к делу число,
        # теперь — только то, что двигает сюжет (подтверждает причину или ей
        # противоречит). Число, которое просто описывает компанию, ушло в аннотацию,
        # и повторять его в тексте незачем.
        #
        # Страховка от вывала осталась прежней: числа всё равно идут подписью, целиком
        # и точно, поэтому у модели нет повода «израсходовать поле».
        out["ГРАНИЦА"] = (
            "Все эти числа УЖЕ уйдут отдельной строкой под постом — пересказывать их "
            "не нужно. В текст можно взять ОДНО, и только если оно СПОРИТ с поводом "
            "или объясняет его причину (например: рейтинг понизили, а долговая "
            "нагрузка за год снизилась). Число, которое просто описывает компанию, "
            "не брать. Остальное объяснять словами, без цифр. "
            "Тезис — чужое мнение: называть с датой."
        )
    return out
