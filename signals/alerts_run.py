...
def compute_value(a: Alert):
    ...
    if a.indicator == "oi_extreme":
        ...
        # threshold = lookback-дни (365/730/1095/1460/1825; 0 = всё время). op = new_high/new_low.
        ...
        for a in alerts:
            ...
            try:
                ...
                if not user:
                    summary["skipped"] += 1
                    continue
                # Алерты по цене актива сняты для всех, кроме админов (решение
                # владельца 2026-09-11, см. api/routers/alerts.py): срабатывание
                # внутри дня раскрывало бы внутридневную цену, а наружу цена идёт
                # только на закрытие 19:00.
                if a.indicator == "price" and (getattr(user, "role", "") or "").lower() != "admin":
                    summary["skipped"] += 1
                    continue
