...
# принималось на чекпоинте +90мин (два чекпоинта, "поздний всплеск" у ~10%
# постов). Найдено 2026-07-15 (замер 78 постов с обоими чекпоинтами): у ВСЕХ 6
# постов, реально пересёкших порог хайпа, fwd_15 УЖЕ был выше ×3 к медиане
# fwd_15-истории — ждать до +90мин не требовалось НИ РАЗУ. Решение переехало на
# +15мин (MTP_CHECKPOINT_1_MIN), baseline считается на fwd_15, не fwd_90.
# MTP_CHECKPOINT_EARLY_MIN — ЕЩЁ более ранний чекпоинт (+3мин), пока ТОЛЬКО для
# измерения (fwd_3 ни на что не влияет) — копим данные, чтобы через несколько
# дней честно ответить, можно ли сократить decision-чекпоинт ниже 15 минут.
# Baseline (медиана fwd_15) считается ОТДЕЛЬНО по каждому каналу
# (tg_channel_watch.channel) — репосты у разных каналов не сравнимы напрямую
# (разный размер аудитории). MTP_CHECKPOINT_*_MIN — окно допуска вокруг
# чекпоинта (крон раз в 5 мин, точного попадания в минуту не будет).
...
MTP_CHECKPOINT_TOLERANCE_MIN = 20   # проверяем в окне [checkpoint, checkpoint+tolerance] —
                                     ...
                                     # мимо окна между двумя тиками
MTP_HYPE_RATIO_MIN = 3.0            # ×N к медиане fwd_15 последних наблюдений — тот же
                                     # ATR-стиль порога, что и у OI/фондов (PUBLIC_RATIO_MIN)
MTP_BASELINE_WINDOW = 50            # сколько последних fwd_15-наблюдений берём для медианы
