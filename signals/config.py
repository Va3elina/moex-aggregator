...
CALENDAR_LOOKAHEAD_DAYS = 30   # на сколько дней вперёд смотрит MOEX-календарь (14 было поздно: кандидат появлялся, когда писать об отсечке уже некогда)
...
MTP_HYPE_RATIO_MIN = 2.0            # ×N к медиане fwd_3 последних наблюдений — тот же
                                     # ATR-стиль порога, что и у OI/фондов (PUBLIC_RATIO_MIN).
                                     # 15.09.2026: было 3.0 — вечером медиана 20-28 давала порог
                                     # 60-85 и резала заметные новости (АдГ 75 репостов при 85.5)
# Абсолютный минимум fwd_3 поверх ×N. Найдено 14.09.2026 (Novo Nordisk меняет
# название, markettwits/384670): утром 8–11 мск медиана fwd_3 падает до 1, и ×3
# превращается в «3 репоста» — проходил любой пост. За 14 дней отсекает 21 из
# 408 промоутов markettwits. У newssmartlab (p50=2) порог 10 срезал бы 174→48,
# поэтому по каналам; канала нет в словаре = без минимума.
MTP_HYPE_MIN_FWD = {"markettwits": 10}
