...
# «рубл» голым корнем не годится: он есть почти в любом посте («млрд рублей»), и channel_wrote 22–24.09
# считал валютным пост про ребаланс фондов индекса. Рубль — только как курс.
LEX = {"валюта": r"доллар|юан|валют|USDRUB|CNYRUB|курс\w* рубл|рубл\w* (?:укреп|ослаб|подешев|подорож)|"
                 r"(?:укреплени|ослаблени)\w* рубл",
...
GLOSS = [
    ...
    (r"за всё время после 2022 года", lambda m: "в посте - просто «исторический максимум»: для канала история - "
                                                "после 2022 года, старые пики в пост не нужны"),
...
def legible(title: str) -> str:
    if "физлиц" in title:
        # «исторический» — после 2022 года (Вадим, 14.09): «максимум с 23.04.2014» у позиции — это
        # рекорд нынешнего рынка; старая дата уводила писателя в «с 2014 года, не за всю историю»
        title = re.sub(r"(максимум|минимум) с (\d\d)\.(\d\d)\.(\d{4})",
                       lambda m: f"{m.group(1)} за всё время после 2022 года" if int(m.group(4)) < 2022 else m.group(0),
                       title)
        for a, b in INSTR:
...
def brief(story: dict) -> tuple:
    ...
    if story["news"]:
        out += ["ПОВОД - новость (в посте подавай событием, без «писали»):"] + [f"- {news_line(n, t)}" for n in story["news"][-2:]] + [""]
