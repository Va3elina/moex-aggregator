...
# ⚠️ Жёсткий потолок на прогон (найдено 08.09.2026). Процесс от 07.09 09:36 провисел
# СУТКИ в epoll на «живом» IPv6-соединении с Telegram: TCP молча умер, Telethon ждёт
# без таймаута, flock выше честно не пускает новые тики — и завод постов сутки не
# получал ни одного кандидата, без единой строки ошибки. Нормальный прогон — секунды,
# 10 минут хватает с запасом; по истечении процесс убивается, замок освобождается,
# следующий тик стартует заново (сессия MTProto переживает обрыв).
exec timeout -k 30 600 /opt/frame/signals/.venv/bin/python -m signals.tg_hype_scan "$@"
